...
def make_task(
    id: EntityId = mid(3), owner: Owner | None = None, status: TaskStatus = TaskStatus.TODO
) -> Task:
    return Task(
        id=id,
        title="Task",
        owner=owner or Owner.of(mid(2)),
        status=status,
        created_at=TS,
        updated_at=TS,
    )
...
def make_knowledge(
    ...
    resources: list[EntityId] | None = None,
) -> Knowledge:
    return Knowledge(
        id=id,
        title="Knowledge",
...
def make_cookbook(
    id: EntityId = mid(5),
    owner: Owner = Owner.kisuke_core(),
    status: CookbookStatus = CookbookStatus.ACTIVE,
) -> Cookbook:
    return Cookbook(
...
def make_review(
    id: EntityId = mid(10), owner: Owner | None = None, status: ReviewStatus = ReviewStatus.PLANNED
) -> Review:
    return Review(
        id=id,
        title="Review",
